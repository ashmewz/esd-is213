                  ...
                  if (!seat) {
                    return (
                      <div
                        ...
                        className="w-8 h-8 shrink-0 rounded-sm border border-gray-200 bg-gray-100"
                      />
                    );
                  }
...
function SeatBottomSheet({ seat, tierCfg, isGA, sectionLabel, onClear, onCheckout }) {
...
export default function SeatmapPage() {
  ...
  const [seatPanelWidth, setSeatPanelWidth] = useState(520);
  ...
  function loadSeatmap({ showLoading = false } = {}) {
    ...
    return getSeatmap(eventId)
      .then((next) => {
        setData(next);
        // If the currently selected seat is no longer available (removed/sold), clear it
        setSelectedSeat((prev) => {
          if (!prev) return null;
          const freshSeat = next?.seats?.find((s) => s.seatId === prev.seatId);
          if (!freshSeat || freshSeat.status === "removed" || freshSeat.status === "sold" || freshSeat.status === "held") {
            return null;
          }
          return prev;
        });
      })
      .catch((e) => { if (showLoading) setError(e.message); })
      .finally(() => {
      ...
      <div className="flex">
        ...
        <main className="flex-1 flex overflow-auto">
          ...
          >
            ...
            {selectedSection && (() => {
              const rawSectionSeats = sectionSeatsById[selectedSection.id] ?? [];
              // Exclude removed seats — they should appear as blank spaces, not dark buttons
              const sectionSeats = rawSectionSeats.filter((s) => String(s.status ?? "").toLowerCase() !== "removed");
              // Only use GA panel if seats have no row structure (true GA — no assigned seats)
              const distinctRows = new Set(sectionSeats.map((s) => s.rowNo)).size;
              const isGA = sectionSeats.length > 0 && distinctRows <= 1 &&
                sectionSeats.every((s) => s.rowNo == null || String(s.rowNo) === "1") &&
                selectedSection.multiline === true &&
                selectedSection.label?.toUpperCase().includes("STANDING");
              if (isGA) {
                return (
                  <GAPanel
